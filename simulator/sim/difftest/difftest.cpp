...
bool isa_difftest_checkregs(CPU_state *ref_r, vaddr_t pc)
{
    ...
    for (int i = 0; i < 32; i++)
    {
        if (ref_r->gpr[i] != sim_cpu.gpr[i])
        {
            printf(ANSI_BG_RED "gpr[%d] is different before executing instruction at pc = " FMT_WORD
                               ", right = " FMT_WORD ", wrong = " FMT_WORD ANSI_NONE "\n",
                   i, pc, ref_r->gpr[i], sim_cpu.gpr[i]);
            return false;
        }
    }
    // check pc
    ...
    {
        printf(ANSI_BG_RED "mcause is different before executing instruction at pc = " FMT_WORD
                           ", right = " FMT_WORD ", wrong = " FMT_WORD ANSI_NONE "\n",
    ...
    {
        printf(ANSI_BG_RED "mepc is different before executing instruction at pc = " FMT_WORD
                           ", right = " FMT_WORD ", wrong = " FMT_WORD ANSI_NONE "\n",
    ...
    {
        printf(ANSI_BG_RED "mstatus is different before executing instruction at pc = " FMT_WORD
                           ", right = " FMT_WORD ", wrong = " FMT_WORD ANSI_NONE "\n",
    ...
    {
        printf(ANSI_BG_RED "mtvec is different before executing instruction at pc = " FMT_WORD
                           ", right = " FMT_WORD ", wrong = " FMT_WORD ANSI_NONE "\n",
